...
def nonrobustify_dataset(args, std_training, train_ds, ):
    with logger.LoggingBlock("Non-Robustifier started", emph=True):
        original_train = []
        non_robust_train = []
        t_train = []
        y_train = []
    ...
    #    saved_path = os.path.join(args.save, 'nonrobustified_' + args.model_name + '_robust_ds' + time.time())
        ...
        temp_train = train_ds.take(792)
        progbar_train = tf.keras.utils.Progbar(len(temp_train))
        # Loops through entire train dataset image-by-image
        for i, (img_batch, label_batch) in enumerate(temp_train):  # Unbatch splits batches into individual images
